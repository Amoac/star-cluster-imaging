...
def get_indices_subhalo(
    ...
    if disrupt_mf > 0:
        indices = array.get_indices(sub['mass.frac.min'], [disrupt_mf, Inf], indices)
    ...
    if ilk == 'all':
        ...
        if 'satellite.prob' in cat:
            ...
            if ilk == 'central.clean.neig':
                ...
                if k in cat:
                    indices = array.get_indices(cat[k], [4, Inf], indices)
                else:
...
def get_indices_in_halo(sub, indices, mass_name='mass.peak', mass_limits=[1, Inf], ilk='satellite'):
    '''
...
def get_indices_tree(cats, ti_start, ti_end, indices=None, get_masks=False):
    ...
    for ti in range(ti_start, ti_end, ti_step):
        iis = array.get_indices(get_indices_tree, [0, Inf])
...
def get_neighbors(
    ...
    neig_mass_frac_limits=[0.25, Inf],
    neig_mass_limits=[],
    neig_number_max=500,
...
class NearestNeighborClass(io.SayClass):
    ...
    def assign_to_self(
        ...
        mass_name='mass',
        mass_limits=[1, Inf],
        neig_mass_frac_limits=[1.0, Inf],
        neig_mass_limits=[1, Inf],
...
def get_catalog_neighbor(
    ...
    gal_mass_limits=[9.7, Inf],
    hal_mass_limits=[],
    ilk='',
    neig_gmass_name='star.mass',
    neig_gmass_limits=[9.7, Inf],
    neig_hmass_limits=[],
    neig_ilk='',
    disrupt_mf=0,
