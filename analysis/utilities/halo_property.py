...
class HaloPropertyClass:
    ...
    def get_nfw_normalization(self, concentrations, overdensity=1):
        ...
        '''
        return overdensity / self.get_nfw_integral_factor(concentrations)

    def get_overdensity_from_linking_length(self, ll=0.168, reference_kind='matter', redshift=None):
        '''
    ...
    def get_overdensity(
        ...
        if 'vir' in virial_kind:
            ...
            if self.Cosmology['omega_curvature'] != 0:
                raise ValueError(
                    'cannot use Bryan & Norman fit for Omega_k = {}'.format(
            ...
            overdensity_value = np.float32(io.get_numbers_in_string(virial_kind, scalarize=True))
    ...
    def _get_nfw_integral_factor_inv(self, radiuss_wrt_scale):
        '''
    ...
    def convert_concentration(
        self, virial_kind_to, virial_kind_from, concentrations_from, redshift=None, solve_kind='fit'
    ):
        '''
        ...
        if 'fof' in virial_kind_to or 'fof' in virial_kind_from:
            ...
            if 'fof' in virial_kind_to and 'fof' not in virial_kind_from:
                density_ratio = (
                    self.get_nfw_normalization(concentrations_from, overdensity_from)
                    / overdensity_to
                )
            ...
            factors = (
                overdensity_from
                / overdensity_to
                / self.get_nfw_integral_factor(concentrations_from)
            )

            if solve_kind == 'spline':
                if self.DensityRadiusSpline is None:
                    self.DensityRadiusSpline = math.SplineFunctionClass(
                        self._get_nfw_integral_factor_inv, [0.01, 100], 500
                    )
                # ensure input concentrations are in spline range
                factors = factors.clip(
                    self.DensityRadiusSpline.fs.min(), self.DensityRadiusSpline.fs.max()
                )
                ...
                concentrations_to = 1 / (
                    (0.5116 * factors ** (2 * p) + 0.5625) ** -0.5 + 2 * factors
                )
    ...
    def get_virial_properties(
        ...
        virdic['mass'] = (
            masses
            * (overdensity_to / overdensity_from) ** 3
            * (virdic['concentration'] / concentrations) ** 3
        )

        virdic['radius'] = (virdic['mass'] / masses * overdensity_from / overdensity_to) ** (
            1 / 3
        ) * halo_radiuss
    ...
    def get_virial_radius(self, virial_kind, masses, redshift=None):
        '''
    ...
    def get_radius_of_zero_acceleration(
        ...
        if scale_to_halo_radius:
            halo_radius = self.get_virial_radius(virial_kind, mass, redshift)
    ...
    def get_density_at_radius(
        ...
        scale_radiuss = self.get_virial_radius(virial_kind, masses, redshift) / concentrations
        ...
        if dimension_number == 3:
            ...
            return 10 ** (
                2
                * normalization
                * integrate.quad(kernel, radiuss * 1.001, radius_end, (scale_radiuss, radiuss))[0]
            )
    ...
    def get_density_within_radius(
        ...
        xs = radiuss / (self.get_virial_radius(virial_kind, masses, redshift) / concentrations)

        return (
            self.get_nfw_normalization(concentrations, overdensity)
            * density_critical
    ...
    def get_density_v_radius(
        ...
        halo_mass = 13  # dummy
        halo_radius = self.get_virial_radius(virial_kind, halo_mass, concentrations)
        ...
        if DistanceBin.dimension_number == 3:
            ...
            mass_in_limit = self.get_mass_within_radius(
                virial_kind, halo_mass, concentrations, radius_limits_normalize[1] * halo_radius
            ) - self.get_mass_within_radius(
                virial_kind, halo_mass, concentrations, radius_limits_normalize[0] * halo_radius
            )
            densities *= (
            ...
            for ri in range(DistanceBin.number):
                densities[ri] = self.get_density_at_radius(
                    ...
                    halo_mass,
                    concentrations,
                    DistanceBin.mins[ri] * halo_radius,
                    DistanceBin.dimension_number,
        ...
        pro = {'density': densities, 'log density': math.get_log(densities)}

        if DistanceBin.log_scale:
            pro['radius'] = math.get_log(rad_bin_values)
        else:
    ...
    def get_virial_velocity(self, virial_kind, masses, redshift=None):
        ...
        vir_radiuss = self.get_virial_radius(virial_kind, masses, redshift)  # [kpc physical]
    ...
    def get_gas_property_virial(self, property_name, virial_kind, masses, redshift=None):
        ...
        # [cm physical]
        vir_radiuss = self.get_virial_radius(virial_kind, masses, redshift) * constant.cm_per_kpc
        vir_vel2s = constant.grav * constant.gram_per_sun * masses / vir_radiuss  # [cm / s]

        if property_name == 'temperature':
            prop_values = (
                molecular_weight * constant.proton_mass / (2 * constant.boltzmann) * vir_vel2s
            )
            ...
            if property_name == 'entropy':
                prop_values = (
                    ...
                    * vir_vel2s
                    / 2
                    / (vir_gas_densities / (molecular_weight * constant.proton_mass)) ** (2 / 3)
                )
            elif property_name == 'pressure':
                prop_values = 1 / 2 * vir_vel2s * vir_gas_densities
        else:
            raise ValueError(f'not recognize property = {property_name}')
