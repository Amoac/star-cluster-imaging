'''
Functions to analyze particle data.
    ...
    if kernel_name == 'newtonian':
        ...
        if property_name == 'acceleration':
            ...
            values = (
                ...
                / (4 * np.pi)
                * kernel_radiuss**2
                / (radiuss**2 + kernel_radiuss**2) ** (3 / 2)
            )
        ...
        if property_name == 'potential':
            ...
            if q < 0.5:
                ...
                values = (
                    -1 / 15 / q**2 + 64 / 3 * q - 48 * q**2 + 192 / 5 * q**3 - 32 / 3 * q**4
                )
            else:
                values = 1 / q**2
...
def get_center_velocities(
    ...
    '''
    Say = io.SayClass(get_center_velocities)
    ...
    if verbose:
        ...
        if center_positions.shape[0] > 1:
            center_string += 's'
        Say.say(
            f'* assigning velocity for {center_positions.shape[0]} {center_string},'
    ...
    for center_i, center_position in enumerate(center_positions):
        ...
        center_velocities[center_i] = coordinate.get_center_velocity(
    ...
    if verbose:
        for center_i, center_velocity in enumerate(center_velocities):
            Say.say(f'host{center_i + 1} velocity = (', end='')
            io.print_array(center_velocity, '{:.1f}', end='')
            print(') [km / s]')
        # print()
...
def get_fof_group_catalog(
    ...
    grp['radius.100'] = np.zeros(group_number, dtype=position_dtype)
    grp['radius.90'] = np.zeros(group_number, dtype=position_dtype)
    grp['radius.50'] = np.zeros(group_number, dtype=position_dtype)
    # center-of-mass position
    grp[position_name] = np.zeros((group_number, positions.shape[1]), dtype=position_dtype)
    if velocities is not None:
        grp['vel.std'] = np.zeros(group_number, dtype=position_dtype)  # 3-D velocity dispersion
    ...
    for gi, indices in enumerate(group_indices):
        ...
        grp[position_name][gi] = np.average(
            positions[indices], 0, masses
        )  # center-of-mass position
        ...
        if 'temperature' in part_spec:
            grp['temperature'][gi] = np.average(part_spec['temperature'][pindices], weights=masses)
    ...
    if has_host:
        ...
        # approximation: do not bother un-doing the Hubble expansion term
...
def get_velocity_dispersion_v_distance(
    ...
    veldisp = {
        ...
        'bin.median': np.zeros((DistanceBin.number, center_positions.shape[0])) - 1,
        'bin.average': np.zeros((DistanceBin.number, center_positions.shape[0])) - 1,
        'cum.median': np.zeros((DistanceBin.number, center_positions.shape[0])) - 1,
        'cum.average': np.zeros((DistanceBin.number, center_positions.shape[0])) - 1,
    }
    ...
    for c_i, center_position in enumerate(center_positions):
        ...
        for d_i in range(DistanceBin.number):
            ...
            if len(part_iis_d) > 1:
                ...
                # use average of velocity^2 (formal standard deviation)
                veldisp['bin.average'][d_i, c_i] = np.sqrt(np.average(velocity2s, weights=weights))
                # use median of velocity^2 (more stable against outliers))
            ...
            if len(part_iis_d) > 1:
                ...
                velocity2s = np.sum((velocities - com_velocity) ** 2, 1)
                # use average of velocity^2 (formal standard deviation)
                veldisp['cum.average'][d_i, c_i] = np.sqrt(np.average(velocity2s, weights=weights))
                # use median of velocity^2 (more stable against outliers))
                veldisp['cum.median'][d_i, c_i] = np.sqrt(
                    math.percentile_weighted(velocity2s, 50, weights)
                )

    for prop in tuple(veldisp):
        if prop != 'distance':
            veldisp[prop + '.2'] = np.percentile(veldisp[prop], 2.275, 1)
            veldisp[prop + '.50'] = np.percentile(veldisp[prop], 50, 1)
            veldisp[prop + '.16'] = np.percentile(veldisp[prop], 16, 1)
            veldisp[prop + '.84'] = np.percentile(veldisp[prop], 84, 1)
            veldisp[prop + '.98'] = np.percentile(veldisp[prop], 97.725, 1)
            # veldisp[prop + '.mean'] = np.mean(veldisp[prop], 1)
            # veldisp[prop + '.std'] = np.std(veldisp[prop], 1)
    ...
    def get_statistics_profiles(
        ...
        for spec_i, spec_name in enumerate(species):
            ...
            if 'velocity' in property_name:
                ...
                for prop_name in pro:
                    if 'velocity' not in prop_name:
                        pros[spec_name][prop_name] = pro[prop_name]
            else:
                prop_values = part[spec_name].prop(property_name, part_indices)
