...
class LabelClass:
    ...
    label_dict = {
        ...
        'size': {'words': 'kernel length', 'symbol': 'h', 'units': '{\\rm pc}'},
    ...
    def get_label(
        ...
        if 'metallicity' in property_name:
            property_name = property_name.replace('.agetracer', '')
            property_name = property_name.replace(' ', '')
        if 'density' in property_statistic:
            property_name = property_statistic

        # convert element names to symbols
        for element in constant.element_symbol_from_name:
            if '.' + element in property_name:
                property_name = property_name.replace(
                    element, constant.element_symbol_from_name[element]
                )
...
def draw_stats(
    ...
    if sigma_number >= 2:
        ...
        subplot.fill_between(
            stat[x_property], stat_lo, stat_hi, alpha=0.1, color=color,
        )
    ...
    if sigma_number >= 1:
        if y_property == 'median':
            ...
            stat_hi = stat['std.lo']
        subplot.fill_between(
            stat[x_property], stat_lo, stat_hi, alpha=0.4, color=color,
        )
...
def make_legends(
    ...
    if len(prop_labels) > 1:
        ...
        _ = subplot.legend(loc=location, fontsize=fontsize)
...
def get_file_name(
    ...
    for name in constant.element_symbol_from_name:
        if name in file_name:
            file_name = file_name.replace(name, constant.element_symbol_from_name[name])
