...
def process_first_snapshot(gs_props, age_cut_Myr=2, age_cut_Min_Myr=1, cluster_select = 0, b_pc=4, ncut_min=5):
    ...
    Parameters:
    - gs_props: Dictionary containing properties of gas and stars in the snapshot.
    - age_cut_Myr: Upper age limit (in Myr) for selecting young stars (default is 2 Myr).
    - age_cut_Min_Myr: Lower age limit (in Myr) for selecting young stars (default is 1 Myr).
    - cluster_select: Index of the cluster to select for returning information (default is 0).
    - b_pc: Binding length in parsecs for the Friends-of-Friends (FoF) algorithm (default is 4 pc).
    - ncut_min: Minimum number of stars for a group to be considered a cluster (default is 5).
...
def compute_velocity_array_xy(xcen, ycen, zcen, coordinates, stepn = 16, pc_width = 40, nkernel = 200, kernel_file_path = '/home1/09528/amoac/'):
    ...
    Parameters:
    - xcen, ycen, zcen: Coordinates of the center point in kpc.
    - coordinates: Dictionary containing the gas particle coordinates, mass, and sizes.
    - stepn: Number of steps (resolution) in each direction for the grid (default is 16).
    - pc_width: Width of the grid in parsecs (default is 40 pc).
    - nkernel: Number of kernel values used for smoothing (default is 200).
    - kernel_file_path: Path to the file containing the kernel values.

    Returns:
    - vel_array_xy: 2D array of velocity values in the xy-plane.
    - res_xy: Resolution of the grid in kpc per step.
    - extent_xy: Extent of the grid in the xy-plane.
    """
...
def compute_velocity_array_xz(xcen, ycen, zcen, coordinates, stepn = 16, pc_width = 40, nkernel = 200, kernel_file_path = '/home1/09528/amoac/'):
    ...
    Returns:
    - vel_array_xz: 2D array of velocity values in the xy-plane.
    - res_xz: Resolution of the grid in kpc per step.
    - extent_xz: Extent of the grid in the xz-plane.
    """
    g_x = coordinates['g_xs']
    g_z = coordinates['g_zs']
    g_mass = coordinates['g_mg']
...
def compute_velocity_array_yz(xcen, ycen, zcen, coordinates, stepn = 16, pc_width = 40, nkernel = 200, kernel_file_path = '/home1/09528/amoac/'):
    ...
    Returns:
    - vel_array_yz: 2D array of velocity values in the xy-plane.
    - res_yz: Resolution of the grid in kpc per step.
    - extent_yz: Extent of the grid in the xy-plane.
    """
    g_y = coordinates['g_ys']
    g_z = coordinates['g_zs']
    g_mass = coordinates['g_mg']
